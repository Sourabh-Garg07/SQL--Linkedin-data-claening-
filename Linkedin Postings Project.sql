...
SELECT job_title, COUNT(job_title) FROM linkedin_2023_cleaned
GROUP BY job_title
ORDER BY 2 DESC;
			-- GRAPHIC DESIGNER HAS HIGHEST JOB POSTINGS (602)

SELECT industry, COUNT(industry) FROM linkedin_2023_cleaned
GROUP BY industry
ORDER BY 2 DESC;
            -- IT SERVICES AND IT CONSULTING INDUSTRY HAS HIGHEST POSTINGS (2403)
            
SELECT company_name, COUNT(company_name) FROM linkedin_2023_cleaned
GROUP BY company_name
ORDER BY 2 DESC;
            -- DIVERSE LYNX COMPANY HAS POST HIGHEST NUMBER OF JOBS (181)

SELECT country, COUNT(country) FROM linkedin_2023_cleaned
GROUP BY country
ORDER BY 2 DESC;    
           -- INDIA HAS HIGHEST JOB POSTINGS (6446)        
            
SELECT SUBSTRING(`date`, 6,2)  AS Months, COUNT(job_title) 
FROM linkedin_2023_cleaned
GROUP BY SUBSTRING(`date`, 6,2)
ORDER BY 1;
          -- APRIL HAS HIGHEST NUMBER OF POSTINGS (7354)
